...
export function NoData() {
  ...
  const snippetConfig = siteMetadata ?? { siteId };
  const dataAttributes = getTrackingScriptDataAttributes(snippetConfig);
  const htmlSnippet = buildTrackingScriptSnippet(snippetConfig);
...
${dataAttributes.map(([key, value]) => `    el.setAttribute("${key}", ${JSON.stringify(value)});`).join("\n")}
  ...
  const inlineScript = `<script src="${scriptUrl}" ${dataAttributes
    .map(formatTrackingScriptAttribute)
    .join(" ")} defer></script>`;
