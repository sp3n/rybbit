...
describe("SessionsService (Redis-backed)", () => {
  ...
  it("keeps trusted server-side events stable across changing fingerprints", async () => {
    mocks.sessionGetOrCreate.mockResolvedValue("x");

    await service.updateSession({
      userId: "device-a",
      identifiedUserId: "employee-alice",
      identifiedUserOnly: true,
      siteId: 42,
    });
    await service.updateSession({
      userId: "device-b",
    ...
    expect(keys[0]).toMatch(/^session:42:identified-server:/);
    expect(keys[0]).not.toContain("employee-alice");
  });

  it("keeps distinct trusted server-side identified users separate", async () => {
    mocks.sessionGetOrCreate.mockResolvedValue("x");
